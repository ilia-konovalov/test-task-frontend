  ...
  <div class="h-screen bg-gray-200 px-4 py-8 sm:px-6 lg:px-8">
    <div class="grid grid-cols-2 grid-rows-fr gap-4 mx-auto">
      <div class="flex flex-wrap overflow-hidden bg-white shadow sm:rounded-lg p-8">
        ...
        <p class="flex basis-full text-sm text-gray-500"> Selected: {{ selectedItemsCount }} / {{ selectedItemsLeftCount }}</p>
      </div>
      <div  class="flex flex-wrap overflow-hidden bg-white shadow sm:rounded-lg p-8">
      ...
      <div class="overflow-hidden bg-white shadow sm:rounded-lg flex flex-wrap flex-col p-8">
        <h3 class="text-lg font-semibold mb-4">Выберите элементы:</h3>
        <div class="flex flex-wrap gap-4">
          <div
            v-for="option in itemsLeft"
            :key="option.id"
            class="flex items-center cursor-pointer border-2 rounded-lg p-2 hover:cursor-pointer"
            :class="{
              'border-indigo-500': selectedItemLeft.some((obj:Item) => obj.id === option.id),
              'border-gray-300': !selectedItemLeft.some((obj:Item) => obj.id === option.id)
            }"
          >
            <input
              type="checkbox"
              :id="`${option.id}`"
              :value="option"
              v-model="selectedItemLeft"
              class="hidden"
            />
            <label :for="`${option.id}`" class="ml-2 text-gray-700 hover:cursor-pointer">{{ option.name }}</label>
          </div>
        </div>
        <button
          type="submit"
          :class="[selectedItemLeft.length ? 'bg-indigo-600' : 'pointer-events-none bg-gray-50 text-gray-200 cursor-not-allowed', 'mt-6 w-full h-12 items-center justify-center rounded-md border border-transparent py-3 px-8 text-base font-medium text-white hover:bg-indigo-700']"
...
let selectedItemLeft = ref<Item[]>([])
let selectedItemRight = ref<Item>()

let selectedItemsLeftCount = computed<number>(() => {
  return itemsLeft.length
})

let selectedItemsCount = computed<number>(() => {
  return selectedItems.value.length
})

function chooseItemLeft():void {
  if(selectedItemLeft.value.length > 0){
    selectedItemLeft.value.forEach(item => {
      let indexToRemove = itemsLeft.findIndex(obj => obj.id === item.id);
      if (indexToRemove !== -1) {
        itemsLeft.splice(indexToRemove, 1);
